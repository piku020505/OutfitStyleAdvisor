    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display tracking-wider uppercase text-xs font-bold text-slate-800">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <button
            ...
            className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-slate-700 hover:border-dynamic-accent hover:text-dynamic-accent transition disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-dynamic-accent"
          >
            <SkipBack size={15} />
          </button>
          ...
          <button
            ...
            className={`rounded-lg border px-3 py-1.5 text-xs font-semibold flex items-center gap-1.5 transition disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-dynamic-accent ${
              isPlaying
                ...
                : 'border-slate-200 bg-slate-50 text-slate-800 hover:border-dynamic-accent'
            }`}
          >
          ...
          <button
            ...
            disabled={loading}
            title="Random Outfit"
            className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-slate-500 hover:border-dynamic-accent hover:text-slate-800 transition disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-dynamic-accent"
          >
            <Shuffle size={15} />
          </button>
        </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 pt-1 no-scrollbar sm:custom-scrollbar">
        {SAMPLE_OUTFITS.map((outfit) => {
          ...
          const swatches = outfit.palette || [outfit.primaryColor, outfit.secondaryColor, outfit.accentColor]
          return (
            <button
              key={outfit.id}
              ...
              className={`group flex-shrink-0 relative rounded-xl border p-3 text-left transition-all duration-200 w-44 sm:w-48 flex flex-col justify-between focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-dynamic-accent
                ${
                  isSelected
                    ? 'border-dynamic-accent bg-slate-100/80 ring-1 ring-dynamic-accent/40 shadow-xs'
            ...
            >
              <div>
                ...
                <h3 className="text-xs font-bold text-slate-900 line-clamp-1 group-hover:text-dynamic-accent transition-colors">
                  {outfit.name}
                </h3>
              </div>
              ...
              <div className="flex items-center gap-1.5 mt-3 pt-2 border-t border-slate-200">
                {swatches.filter(Boolean).map((hex) => (
                  <span
                    key={hex}
